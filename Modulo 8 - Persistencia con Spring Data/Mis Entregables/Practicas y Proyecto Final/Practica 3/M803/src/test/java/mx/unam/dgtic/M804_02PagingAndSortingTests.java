...
public class M804_02PagingAndSortingTests {
    ...
    final String ALUMNO = "ALEJANDRO NOYOLA";
    private static final int PAGE_SIZE = 10;


    @Autowired
    AlumnoPagingAndSortingRepository repositoioAlumno;
    ...
    void BuscarTodosPaginaTest(){
        ...
        Page<Alumno> alumnoPage = repositoioAlumno.findAll(pagina);

        System.out.println("Pagina 1 de " + alumnoPage.getTotalPages());
        alumnoPage.forEach(System.out::println);

        pagina = PageRequest.of(3, PAGE_SIZE,
                Sort.by("nombre").ascending());
        alumnoPage = repositoioAlumno.findAll(pagina);
    ...
    void BuscarTodosPaginaWhileTest(){
        ...
        do {
            alumnoPage = repositoioAlumno.findAll(pagina);
            System.out.println("Pagina " + (pagina.getPageNumber() +1) + " de " + alumnoPage.getTotalPages());
            alumnoPage.forEach(System.out::println);
    ...
    void buscarAlumnosOrdenadosTest(String campo){
        System.out.println(ALUMNO);
        System.out.println("Ordenando por " + campo);
        Sort sort = Sort.by(Sort.Direction.ASC, campo);
        repositoioAlumno.findAll(sort).forEach(System.out::println);
    }


    @ParameterizedTest
    @ValueSource(strings = {"nombre", "paterno", "estatura", "fnac"})
    void buscarAlumnosTest(String campo){
        System.out.println(ALUMNO);
        System.out.println("Ordenando por " + campo);
        Sort sort = Sort.by(Sort.Direction.ASC, campo);
        repositoioAlumno.findAll(sort).forEach(System.out::println);
